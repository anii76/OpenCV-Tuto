import cv2
import numpy as np

#matrix of zeros => black img
img = np.zeros((512,512,3),np.uint8)

print(img.shape)
#blue, (height,width) (B,G,R)
img[200:300, 100:300] = 255,0,0

#lines
#src_img, starting pont coordinates(height,width), ending , color , thikness
#cv2.line(img, (0,0), (300,300), (0,255,0), 3)
cv2.line(img, (img.shape[1]//2,0), (img.shape[1]//2,img.shape[0]), (0,255,0),1)
cv2.line(img, (0,img.shape[0]//2), (img.shape[1],img.shape[0]//2), (0,255,0),1)

#rectangle
cv2.rectangle(img, (0,0), (250,300), (0,0,255), 2)

#circle
#img , center, radius, color, thickness
cv2.circle(img, (400,50), 30, (255,255,0), 5)

#img , text, pos , font , scale , color , thickness
cv2.putText(img, " OPENCV ",(300,200), cv2.FONT_HERSHEY_COMPLEX, 1, (0,150,0), 1)

cv2.imshow("Black" , img)

cv2.waitKey(0)